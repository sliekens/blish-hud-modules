...
{
    public static readonly TimeoutStrategyOptions TotalTimeoutStrategy = new() { Timeout = TimeSpan.FromMinutes(3) };
    ...
    public static readonly RetryStrategyOptions<HttpResponseMessage> RetryStrategy = new()
    {
        ...
        ShouldHandle = static async attempt => attempt.Outcome switch
        {
            ...
            { Result.StatusCode: (HttpStatusCode)422 } => true,
    ...
    public static readonly TimeoutStrategyOptions AttemptTimeoutStrategy = new() { Timeout = TimeSpan.FromSeconds(30) };
